...
#define ImGui_Renderer_Texture(texture, viewport_x, viewport_y, tex_x, tex_y)                                                  \
    ImGui::Image(                                                                                                              \
        (void *)(uint64_t)((OpenGLRenderTexture *)texture.get())->get_renderer_id(), ImVec2(viewport_x, viewport_y),           \
        ImVec2(0, tex_y), ImVec2(tex_x, 0))
...
ImTextureID imgui_vulkan_texture_id(Yogi::VulkanRenderTexture *texture)
{
    ...
    g_TextureMap[texture] = ImGui_ImplVulkan_AddTexture(
        texture->get_vk_sampler(), texture->get_vk_image_view(), VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);
...
#define ImGui_Renderer_Texture(texture, viewport_x, viewport_y, tex_x, tex_y)                                                  \
    ImGui::Image(                                                                                                              \
        imgui_vulkan_texture_id((VulkanRenderTexture *)texture.get()), ImVec2(viewport_x, viewport_y), ImVec2(0, tex_y),       \
        ImVec2(tex_x, 0))
