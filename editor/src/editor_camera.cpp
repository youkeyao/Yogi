...
{
    if (m_camera_component.is_ortho)
        m_camera_projection_matrix = glm::ortho(
            -m_camera_component.aspect_ratio * m_camera_component.zoom_level,
            m_camera_component.aspect_ratio * m_camera_component.zoom_level, -m_camera_component.zoom_level,
            m_camera_component.zoom_level, -1.0f, 1.0f);
    else
...
void EditorCamera::recalculate_view()
{
    glm::mat4 &transform = (glm::mat4 &)m_perspective_transform_component.transform;
    if (m_camera_component.is_ortho) {
        m_ortho_transform_component.transform =
            glm::translate(glm::mat4(1.0f), glm::vec3{ transform[3][0], transform[3][1], 0 });
    ...
    if (Input::is_mouse_button_pressed(YG_MOUSE_BUTTON_RIGHT)) {
        ...
        if (m_camera_component.is_ortho) {
            transform = glm::translate(
                transform, { (m_last_mouse_x - e.get_x()) / m_pixel_ratio, (e.get_y() - m_last_mouse_y) / m_pixel_ratio, 0 });
        } else {
            transform *= glm::rotate(
                glm::mat4(1.0f), glm::radians(m_last_mouse_y - e.get_y()) / m_pixel_ratio * 40.0f, glm::vec3{ 1.0f, 0, 0 });
            transform *= glm::rotate(
                glm::mat4(1.0f), glm::radians(m_last_mouse_x - e.get_x()) / m_pixel_ratio * 40.0f,
                glm::inverse(glm::mat3(transform)) * glm::vec3{ 0, 1.0f, 0 });
        }
        recalculate_view();
        m_last_mouse_x = e.get_x();
        m_last_mouse_y = e.get_y();
