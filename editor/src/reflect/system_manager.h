...
{
private:
    typedef void (*SystemFunc)(const Ref<Scene> &, const std::string &);
...
public:
    static void init();
    static void clear();
    static void add_system(const Ref<Scene> &scene, const std::string &system_name);
    static void remove_system(const Ref<Scene> &scene, const std::string &system_name);
    ...
    static void register_system()
    {
        ...
        s_add_system_funcs[system_name] = [](const Ref<Scene> & scene, const std::string &system_name) {
            scene->add_system<Type>();
        };
        s_remove_system_funcs[system_name] = [](const Ref<Scene> & scene, const std::string &system_name) {
    ...
    {
        s_runtime_systems[system_name] = CreateRef<RuntimeSystem>(update_function, event_function);
        s_add_system_funcs[system_name] = [](const Ref<Scene> & scene, const std::string &system_name) {
            scene->add_runtime_system(system_name, s_runtime_systems[system_name]);
        };
        s_remove_system_funcs[system_name] = [](const Ref<Scene> & scene, const std::string &system_name) {
...
private:
    ...
    static std::unordered_map<std::string, Ref<RuntimeSystem>> s_runtime_systems;
};
