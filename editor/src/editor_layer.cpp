...
namespace Yogi {
    ...
    void EditorLayer::on_attach()
    {
        ...
        square->add_component("TagComponent");
        ComponentManager::field<std::string>(square->get_component("TagComponent"), "TagComponent", "tag") = "square";
        square->add_component("TransformComponent");
        ComponentManager::field<glm::vec3>(square->get_component("TransformComponent"), "TransformComponent", "scale") = glm::vec3(1.0f);
        square->add_component("SpriteRendererComponent");
        void* sprite_component = square->get_component("SpriteRendererComponent");
        ComponentManager::field<glm::vec4>(sprite_component, "SpriteRendererComponent", "color") = { 0.8f, 0.2f, 0.3f, 1.0f };

        m_editor_camera = m_scene->create_entity();
        m_editor_camera->add_component("TagComponent");
        ComponentManager::field<std::string>(m_editor_camera->get_component("TagComponent"), "TagComponent", "tag") = "camera";
        m_editor_camera->add_component("TransformComponent");
        m_editor_camera->add_component("CameraComponent");
        ComponentManager::field<float>(m_editor_camera->get_component("CameraComponent"), "CameraComponent", "aspect_ratio") = 1.0f;
        ComponentManager::field<float>(m_editor_camera->get_component("CameraComponent"), "CameraComponent", "zoom_level") = 1.0f;
