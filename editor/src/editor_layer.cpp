...
void EditorLayer::on_update(Timestep ts)
{
    ...
    if (m_scene_state == SceneState::Edit) {
        ...
        RenderCommand::clear();
        m_scene->view_components<TransformComponent, MeshRendererComponent>(
            [&](Entity entity, TransformComponent &transform, MeshRendererComponent &mesh_renderer) {
...
void EditorLayer::imgui_update()
{
    ...
    glm::vec2 new_viewport_size = { viewport_region_max.x - viewport_region_min.x,
                                    viewport_region_max.y - viewport_region_min.y };
    ...
    ImguiSetting::show_image(m_frame_texture, ImVec2(m_viewport_size.x, m_viewport_size.y), ImVec2(1, 1));
    ...
    if (selected_entity && m_scene_state == SceneState::Edit) {
        ...
        ImGuizmo::Manipulate(
            glm::value_ptr(camera_view), glm::value_ptr(camera_projection), m_gizmo_type, ImGuizmo::LOCAL,
            glm::value_ptr((glm::mat4 &)tc.transform), nullptr, snap ? snap_values : nullptr);
    }
    ImGui::End();
