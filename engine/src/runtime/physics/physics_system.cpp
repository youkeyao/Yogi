...
public:
    ...
    virtual const char *GetBroadPhaseLayerName(JPH::BroadPhaseLayer inLayer) const override
    {
        ...
        case (JPH::BroadPhaseLayer::Type)BroadPhaseLayers::MOVING:
            return "MOVING";
        default:
            YG_CORE_ASSERT(false, "Invalid BroadPhaseLayer!");
            return "INVALID";
        }
    }
...
PhysicsSystem::PhysicsSystem()
{
    ...
    m_physics_system->Init(
        cMaxBodies, cNumBodyMutexes, cMaxBodyPairs, cMaxContactConstraints, *m_broad_phase_layer_interface,
    ...
    scene->view_components<TransformComponent, RigidBodyComponent>(
        [&](Entity entity, TransformComponent &transform, RigidBodyComponent &rigid_body) {
            ...
            transform.set_world_transform(glm::mat4(
                transform_matrix(0, 0) * scale.x, transform_matrix(1, 0) * scale.x, transform_matrix(2, 0) * scale.x,
                transform_matrix(3, 0), transform_matrix(0, 1) * scale.y, transform_matrix(1, 1) * scale.y,
                transform_matrix(2, 1) * scale.y, transform_matrix(3, 1), transform_matrix(0, 2) * scale.z,
                transform_matrix(1, 2) * scale.z, transform_matrix(2, 2) * scale.z, transform_matrix(3, 2),
