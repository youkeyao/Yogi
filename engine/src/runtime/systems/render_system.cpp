...
namespace Yogi {
    ...
    std::vector<glm::mat4> pointShadowTransforms = {
        glm::perspective(glm::radians(90.0f), 1.0f, near, far) * glm::lookAt(glm::vec3(0.0f), glm::vec3(1.0f, 0.0f, 0.0f), glm::vec3(0.0f, -1.0f, 0.0f)),
        glm::perspective(glm::radians(90.0f), 1.0f, near, far) * glm::lookAt(glm::vec3(0.0f), glm::vec3(-1.0f, 0.0f, 0.0f), glm::vec3(0.0f, -1.0f, 0.0f)),
        glm::perspective(glm::radians(90.0f), 1.0f, near, far) * glm::lookAt(glm::vec3(0.0f), glm::vec3(0.0f, 1.0f, 0.0f), glm::vec3(0.0f, 0.0f, 1.0f)),
        glm::perspective(glm::radians(90.0f), 1.0f, near, far) * glm::lookAt(glm::vec3(0.0f), glm::vec3(0.0f, -1.0f, 0.0f), glm::vec3(0.0f, 0.0f, -1.0f)),
        glm::perspective(glm::radians(90.0f), 1.0f, near, far) * glm::lookAt(glm::vec3(0.0f), glm::vec3(0.0f, 0.0f, 1.0f), glm::vec3(0.0f, -1.0f, 0.0f)),
        glm::perspective(glm::radians(90.0f), 1.0f, near, far) * glm::lookAt(glm::vec3(0.0f), glm::vec3(0.0f, 0.0f, -1.0f), glm::vec3(0.0f, -1.0f, 0.0f))
    };
    ...
    void RenderSystem::set_light(Scene* scene)
    {
        ...
        scene->view_components<TransformComponent, DirectionalLightComponent>([&](Entity entity, TransformComponent& transform, DirectionalLightComponent& light){
            ...
            const glm::mat4 light_space_matrix = glm::ortho(-10.0f, 10.0f, -10.0f, 10.0f, -1.0f, 10.0f) * glm::inverse(world_transform);
            const uint32_t shadow_pool_index = allocate_shadow_frame_buffer();
            m_directional_lights.emplace_back(std::make_pair(light, ShadowData{light_space_matrix, world_transform, shadow_pool_index}));
            ...
            RenderCommand::clear();
            scene->view_components<TransformComponent, MeshRendererComponent>([&](Entity mesh_entity, TransformComponent& mesh_transform, MeshRendererComponent& mesh_renderer){
                if (mesh_renderer.cast_shadow) {
                    Renderer::draw_mesh(mesh_renderer.mesh, MaterialManager::get_material("shadow"), mesh_transform.get_world_transform(), mesh_entity);
                }
            });
            Renderer::flush();
        ...
        scene->view_components<TransformComponent, SpotLightComponent>([&](Entity entity, TransformComponent& transform, SpotLightComponent& light){
            const glm::mat4 world_transform = transform.get_world_transform();
            const glm::mat4 light_space_matrix = glm::perspective(glm::radians(light.outer_angle * 2), 1.0f, near, far) * glm::inverse(world_transform);
            const uint32_t shadow_pool_index = allocate_shadow_frame_buffer();
        ...
        scene->view_components<TransformComponent, PointLightComponent>([&](Entity entity, TransformComponent& transform, PointLightComponent& light){
            std::array<ShadowData, 6>& data = m_point_lights.emplace_back(std::make_pair(light, std::array<ShadowData, 6>{})).second;
    ...
    void RenderSystem::render_camera(const CameraComponent& camera, const TransformComponent& transform, Scene* scene)
    {
        ...
        if (camera.is_ortho)
            Renderer::set_projection_view_matrix(glm::ortho(-camera.aspect_ratio * camera.zoom_level, camera.aspect_ratio * camera.zoom_level, -camera.zoom_level, camera.zoom_level, -1.0f, 1.0f) * glm::inverse((glm::mat4)transform.get_world_transform()));
        else
            Renderer::set_projection_view_matrix(glm::perspective(camera.fov, camera.aspect_ratio, 0.1f, 500.0f) * glm::inverse((glm::mat4)transform.get_world_transform()));
        ...
        RenderCommand::clear();
        {
            std::vector<std::pair<DirectionalLightComponent, ShadowData>>::iterator directional_light_iterator = m_directional_lights.begin();
            std::vector<std::pair<SpotLightComponent, ShadowData>>::iterator spot_light_iterator = m_spot_lights.begin();
            std::vector<std::pair<PointLightComponent, std::array<ShadowData, 6>>>::iterator point_light_iterator = m_point_lights.begin();
            while (directional_light_iterator != m_directional_lights.end() || spot_light_iterator != m_spot_lights.end() || point_light_iterator != m_point_lights.end()) {
                ...
                if (directional_light_iterator != m_directional_lights.end()) {
                    ...
                    Renderer::set_directional_light(directional_light.color, glm::vec3{(glm::mat4)shadow_data.light_transform * glm::vec4(0, 0, -1, 0)}, shadow_data.light_space_matrix);
                ...
                while (spot_light_iterator != m_spot_lights.end()) {
                    ...
                    if (!Renderer::add_spot_light({
                        spot_light.color,
                        glm::vec3{(glm::mat4)shadow_data.light_transform * glm::vec4(0, 0, 0, 1)},
                        glm::cos(glm::radians(spot_light.inner_angle)),
                        glm::vec3{(glm::mat4)shadow_data.light_transform * glm::vec4(0, 0, -1, 0)},
                        glm::cos(glm::radians(spot_light.outer_angle)),
                ...
                while (point_light_iterator != m_point_lights.end()) {
                    ...
                    if (!Renderer::add_point_light({
                        glm::vec3{(glm::mat4)shadow_data[0].light_transform * glm::vec4(0, 0, 0, 1)},
                        point_light.attenuation_parm,
                        point_light.color,
                        shadow_data[0].light_space_matrix
                    })) break;
                    for (int32_t i = 0; i < 6; i ++) {
                        Renderer::add_texture(m_shadow_frame_buffer_pool[shadow_data[i].shadow_pool_index]->get_color_attachment(0));
                    }
                    point_light_iterator ++;
                }
                Renderer::set_texture_init_slot(Renderer::get_current_texture_slot());
                scene->view_components<TransformComponent, MeshRendererComponent>([&](Entity mesh_entity, TransformComponent& mesh_transform, MeshRendererComponent& mesh_renderer){
                    Renderer::draw_mesh(mesh_renderer.mesh, mesh_renderer.material, mesh_transform.get_world_transform(), mesh_entity);
                });
                Renderer::flush();
            }
        }
        ...
        if (s_frame_buffer) s_frame_buffer->unbind();
    }
