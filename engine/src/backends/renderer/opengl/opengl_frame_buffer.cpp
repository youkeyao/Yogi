    ...
    : m_width(width), m_height(height), m_color_attachments(color_attachments)
{
    ...
    for (int32_t i = 0; i < color_attachments.size(); i++) {
        ...
        glFramebufferTexture2D(
            ...
            ((OpenGLRenderTexture *)attachment.get())->get_renderer_id(), 0);
    ...
    for (int32_t i = 0; i < color_attachments.size(); i++) {
        ...
        glRenderbufferStorageMultisample(
            GL_RENDERBUFFER, context->get_msaa_samples(), ((OpenGLRenderTexture *)attachment.get())->get_internal_format(),
            m_width, m_height);
...
void OpenGLFrameBuffer::resize(uint32_t width, uint32_t height)
{
    ...
    for (int32_t i = 0; i < m_color_attachments.size(); i++) {
        ...
        glRenderbufferStorageMultisample(
            GL_RENDERBUFFER, context->get_msaa_samples(),
            ((OpenGLRenderTexture *)m_color_attachments[i].get())->get_internal_format(), m_width, m_height);
        glBindRenderbuffer(GL_RENDERBUFFER, 0);
    }
}
