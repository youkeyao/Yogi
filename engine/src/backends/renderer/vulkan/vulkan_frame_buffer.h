...
public:
    VulkanFrameBuffer(
        uint32_t width, uint32_t height, const std::vector<Ref<RenderTexture>> &color_attachments, bool is_msaa = true,
        bool has_depth_attachment = true, VkImageLayout layout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);
    ~VulkanFrameBuffer();
