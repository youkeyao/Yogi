...
void VulkanContext::create_buffer(
...
void VulkanContext::create_image(
...
void VulkanContext::transition_image_layout(
    VkImage image, VkFormat format, VkImageLayout old_layout, VkImageLayout new_layout, VkImageAspectFlags aspect_flags)
{
    VkCommandBuffer commandBuffer = begin_single_time_commands();
...
void VulkanContext::create_swap_chain()
{
    ...
    for (size_t i = 0; i < m_swap_chain_images.size(); i++) {
        m_swap_chain_textures[i] = CreateRef<VulkanRenderTexture>(
            m_swap_chain_extent.width, m_swap_chain_extent.height, m_swap_chain_images[i], m_swap_chain_image_format);
    }
}

VkRenderPass VulkanContext::create_render_pass(
    const std::vector<VkFormat> &color_attachment_formats, VkImageLayout init_layout, VkImageLayout final_layout, bool is_msaa,
    bool has_depth_attachment)
{
    ...
    for (auto &format : color_attachment_formats) {
        ...
        colorAttachment.loadOp =
            init_layout == VK_IMAGE_LAYOUT_UNDEFINED ? VK_ATTACHMENT_LOAD_OP_CLEAR : VK_ATTACHMENT_LOAD_OP_LOAD;
    ...
    if (has_depth_attachment) {
        ...
        depthAttachment.initialLayout = init_layout == VK_IMAGE_LAYOUT_UNDEFINED ?
...
void VulkanContext::create_frame_buffers(bool is_msaa, bool has_depth_attachment)
{
    ...
    for (size_t i = 0; i < m_swap_chain_images.size(); i++) {
        m_swap_chain_frame_buffers[i] = CreateRef<VulkanFrameBuffer>(
            m_swap_chain_extent.width, m_swap_chain_extent.height, std::vector<Ref<RenderTexture>>{ m_swap_chain_textures[i] },
            is_msaa, has_depth_attachment, VK_IMAGE_LAYOUT_PRESENT_SRC_KHR);
    }
}
...
void VulkanContext::create_sync_objects()
{
    ...
    for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
        YG_CORE_ASSERT(
            (vkCreateSemaphore(m_device, &semaphoreInfo, nullptr, &m_image_available_semaphores[i]) == VK_SUCCESS &&
...
VkFormat VulkanContext::find_supported_format(
...
void VulkanContext::begin_frame()
{
    ...
    if (!m_current_frame_buffer && m_image_index < 0) {
        VkResult result = vkAcquireNextImageKHR(
            m_device, m_swap_chain, UINT64_MAX, m_image_available_semaphores[m_current_frame], VK_NULL_HANDLE,
