...
VulkanVertexBuffer::VulkanVertexBuffer(void *vertices, uint32_t size, bool is_static)
{
    ...
    context->create_buffer(
        size, VK_BUFFER_USAGE_VERTEX_BUFFER_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT,
...
VulkanIndexBuffer::VulkanIndexBuffer(uint32_t *indices, uint32_t count, bool is_static)
{
    ...
    context->create_buffer(
        size, VK_BUFFER_USAGE_INDEX_BUFFER_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT,
        m_buffer, m_buffer_memory);
    vkMapMemory(context->get_device(), m_buffer_memory, 0, size, 0, &m_buffer_mapped);
    if (indices)
        set_data(indices, size);
}
