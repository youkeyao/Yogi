...
VulkanPipeline::VulkanPipeline(const std::string &name, const std::vector<std::string> &types)
{
    ...
    if (!layout_bindings.empty()) {
        ...
        for (int32_t i = 0; i < layout_bindings.size(); i++) {
            ...
            VkResult result =
                vkCreateDescriptorSetLayout(context->get_device(), &layout_info, nullptr, &m_descriptor_set_layouts[i]);
    ...
    multisampling.rasterizationSamples = context->get_msaa_samples();
    ...
    VkRenderPass render_pass =
        context->create_render_pass(color_attachment_formats, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_PRESENT_SRC_KHR);
...
void VulkanPipeline::reflect_vertex(
...
void VulkanPipeline::reflect_uniform_buffer(
    ...
    VkShaderStageFlagBits stage_flag, uint32_t &ubo_count)
{
    ...
    for (auto &uniform_buffer : resources.uniform_buffers) {
        ...
        while (ubo_layout_bindings.size() <= set)
            ubo_layout_bindings.push_back(std::vector<VkDescriptorSetLayoutBinding>{});
        while (ubo_layout_bindings[set].size() <= binding)
            ubo_layout_bindings[set].push_back(VkDescriptorSetLayoutBinding{});
...
void VulkanPipeline::reflect_sampler(
    const spirv_cross::CompilerGLSL &compiler, std::vector<std::vector<VkDescriptorSetLayoutBinding>> &sampler_layout_bindings,
    VkShaderStageFlagBits stage_flag, uint32_t &sampler_count)
{
    spirv_cross::ShaderResources resources = compiler.get_shader_resources();
    ...
    for (auto &sampled_image : resources.sampled_images) {
        ...
        while (sampler_layout_bindings.size() <= set)
            sampler_layout_bindings.push_back(std::vector<VkDescriptorSetLayoutBinding>{});
        while (sampler_layout_bindings[set].size() <= binding)
            sampler_layout_bindings[set].push_back(VkDescriptorSetLayoutBinding{});
