...
Ref<FrameBuffer> FrameBuffer::create(
    uint32_t width, uint32_t height, const std::vector<Ref<RenderTexture>> &color_attachments, bool has_depth_attachment)
{
    return CreateRef<VulkanFrameBuffer>(width, height, color_attachments, has_depth_attachment);
}

VulkanFrameBuffer::VulkanFrameBuffer(
    uint32_t width, uint32_t height, const std::vector<Ref<RenderTexture>> &color_attachments, bool is_msaa,
    bool has_depth_attachment, VkImageLayout layout)
    : m_width(width), m_height(height), m_color_attachments(color_attachments), m_is_msaa(is_msaa),
    ...
    for (int32_t i = 0; i < color_attachments.size(); i++) {
        ...
        if (m_is_msaa) {
            context->create_image(
                width, height, context->get_msaa_samples(), attachment_formats[i], VK_IMAGE_TILING_OPTIMAL,
                VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT | VK_IMAGE_USAGE_INPUT_ATTACHMENT_BIT | VK_IMAGE_USAGE_SAMPLED_BIT,
                VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, m_msaa_images[i], m_msaa_image_memories[i]);
            m_msaa_image_views[i] =
                context->create_image_view(m_msaa_images[i], attachment_formats[i], VK_IMAGE_ASPECT_COLOR_BIT);
        }
    }

    m_clear_render_pass =
        context->create_render_pass(attachment_formats, VK_IMAGE_LAYOUT_UNDEFINED, layout, is_msaa, has_depth_attachment);
...
void VulkanFrameBuffer::create_vk_frame_buffer(const std::vector<VkImageView> &image_views)
{
    ...
    if (m_has_depth_attachment) {
        ...
        context->create_image(
            m_width, m_height, context->get_msaa_samples(), depth_format, VK_IMAGE_TILING_OPTIMAL,
        ...
        context->transition_image_layout(
            m_depth_image, depth_format, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_DEPTH_STENCIL_ATTACHMENT_OPTIMAL,
...
void VulkanFrameBuffer::resize(uint32_t width, uint32_t height)
{
    ...
    if (m_is_msaa) {
        ...
        for (int32_t i = 0; i < m_color_attachments.size(); i++) {
            ...
            context->create_image(
                width, height, context->get_msaa_samples(), attachment_format, VK_IMAGE_TILING_OPTIMAL,
                VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT | VK_IMAGE_USAGE_INPUT_ATTACHMENT_BIT | VK_IMAGE_USAGE_SAMPLED_BIT,
                VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, m_msaa_images[i], m_msaa_image_memories[i]);
