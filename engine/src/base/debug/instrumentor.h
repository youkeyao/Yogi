	...
	public:
		...
		void begin_session(const std::string& name, const std::string& filepath = "results.json")
		{
			...
			if (m_current_session) {
                HZ_CORE_ERROR("Instrumentor::BeginSession('{0}') when session '{1}' already open.", name, m_current_session->name);
...
#define HZ_PROFILE 0
	...
	#define HZ_PROFILE_BEGIN_SESSION(name, filepath) ::hazel::Instrumentor::get().begin_session(name, filepath)
	#define HZ_PROFILE_END_SESSION() ::hazel::Instrumentor::get().end_session()
