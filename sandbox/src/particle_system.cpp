...
public:
    ...
    static float rand() { return (float)s_distribution(s_random_engine) / (float)std::numeric_limits<uint32_t>::max(); }
...
//         // Yogi::Renderer2D::draw_quad({particle.position.x, particle.position.y, 0.0f}, particle.rotation, {size, size},
//         color);
//     }
