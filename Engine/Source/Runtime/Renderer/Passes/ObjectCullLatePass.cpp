...
ObjectCullLatePass::ObjectCullLatePass()
{
    WRef<ShaderDesc> shader = AssetManager::AcquireAsset<ShaderDesc>(ObjectCull::SHADER_LATE);
    PipelineDesc     desc{};
...
void ObjectCullLatePass::Prepare(RenderGraphContext& context, RenderGraph& graph, RenderGraphBuilder& builder)
{
    ...
    builder.UseBuffer(m_objectVis[0].Get(), ResourceState::UnorderedAccess);
    builder.UseBuffer(m_objectVis[1].Get(), ResourceState::UnorderedAccess);
    ...
    builder.UseTexture(m_depthPyramid, ResourceState::UnorderedAccess | ResourceState::ComputeShaderResource);
}

void ObjectCullLatePass::Execute(RenderGraphContext& context)
{
    if (!context.Buckets || !m_depthPyramid || context.TotalDrawCount == 0)
        return;
    ...
    for (size_t bi = 0; bi < context.Buckets->size(); ++bi)
    {
        ...
        ExecuteBucket(context.CommandBuffer,
                      context.SceneFrameAddr,
                      b,
                      pyramidSlot,
...
void ObjectCullLatePass::ExecuteBucket(ICommandBuffer*   cmd,
                                       ...
                                       uint32_t          writeIdx)
{
    ...
    cmd->BeginDebugLabel("ObjectCullLate::Execute");
