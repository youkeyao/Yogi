...
WRef<IPipeline> MeshletDrawLatePass::BuildPipeline(const std::string& shaderKey)
{
    ...
    {
        YG_CORE_ERROR(
            "MeshletDrawLatePass: shader load failed for type '{0}' (shaderKey='{1}')", materialTypeName, shaderKey);
        return nullptr;
    }
...
WRef<IPipeline> MeshletDrawLatePass::AcquirePipeline(const std::string& shaderKey)
{
    if (shaderKey.empty())
    {
        YG_CORE_WARN("MeshletDrawLatePass::AcquirePipeline called with empty shaderKey");
        return nullptr;
    }
...
void MeshletDrawLatePass::Prepare(RenderGraphContext& context, RenderGraph& graph, RenderGraphBuilder& builder)
{
    ...
    m_depthPyramid           = graph.GetTexture("HiZ.DepthPyramid");
    ...
    if (!m_depthPyramid || context.TotalDrawCount == 0)
        return;
    ...
    if (m_depthPyramid)
    {
        builder.UseTexture(m_depthPyramid,
...
void MeshletDrawLatePass::Execute(RenderGraphContext& context)
{
    if (!context.Buckets || !context.CurrentTarget || !context.DepthView || !m_depthPyramid ||
        context.TotalDrawCount == 0)
        return;
    ...
    const uint32_t pyramidSlot = BindlessTextureManager::Find(m_depthPyramid);
    if (pyramidSlot == BindlessTextureManager::INVALID_SLOT)
        return;
    ...
    for (size_t bi = 0; bi < context.Buckets->size(); ++bi)
    {
        ...
        ExecuteBucket(cmd, context.SceneFrameAddr, b, pyramidSlot, static_cast<uint32_t>(bi), m_readIdx, m_writeIdx);
    }
    cmd->EndRendering();
}

void MeshletDrawLatePass::ExecuteBucket(ICommandBuffer*   cmd,
                                        ...
                                        uint32_t          writeIdx)
{
    ...
    cmd->BeginDebugLabel("MeshletDrawLate::Execute");
